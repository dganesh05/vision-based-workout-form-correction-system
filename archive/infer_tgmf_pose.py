...
from archive.pose_json_bridge import DEFAULT_NUM_JOINTS, convert_json_to_tvc
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run TGMF-Pose 3D lifting from standardized 2D keypoints. "
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Python file that exposes a function run_tgmf_pose_inference(sequence_2d, checkpoint, device, prompt)."
        ),
    )
    parser.add_argument(
        "--adapter-fn",
        default="run_tgmf_pose_inference",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to pretrained TGMF-Pose checkpoint",
    )
    parser.add_argument("--device", default="cpu", help="Torch device string, e.g. cpu or cuda:0")
    parser.add_argument(
        "--prompt",
        default="A person performing a barbell squat.",
        help="Text prompt for text-guided temporal lifting",
    )
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("outputs/poses_3d/tgmf_pose_output.npy"),
        help="Output .npy path for 3D coordinates",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("outputs/poses_3d/tgmf_pose"),
...
def main() -> None:
    ...
    for source_type, source_path in input_items:
        ...
        if sequence_3d.shape[:2] != sequence_2d.shape[:2]:
            raise ValueError(
                "TGMF-Pose adapter returned mismatched T/V dimensions: "
                f"input={sequence_2d.shape}, output={sequence_3d.shape}"
            )
        if sequence_3d.shape[2] != 3:
            raise ValueError(
                "TGMF-Pose adapter must return channels (x,y,z), so output C must equal 3. "
                f"Got shape {sequence_3d.shape}"
            )

        if batch_mode:
            out_path = args.output_dir / f"{source_path.stem}_tgmf_pose.npy"
        else:
            out_path = args.output

        out_path.parent.mkdir(parents=True, exist_ok=True)
        np.save(out_path, sequence_3d)
        print(f"Saved TGMF-Pose output to {out_path} with shape {sequence_3d.shape}")
